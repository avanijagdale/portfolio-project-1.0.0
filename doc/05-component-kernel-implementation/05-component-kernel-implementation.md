...
I think the best choice to represent an inventory component would probably a map, because
an inventory requires each unique item to be matched with it's corresponding quantity, and
a map would make sense to store information where each entry has two relevant pieces of data.
With a map, the name of the item could be stored as a key, and it's corresponding quantity
could be stored as the value.
