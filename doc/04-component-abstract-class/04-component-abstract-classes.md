...
- I added a kernel method items() to get a list of all the items in the inventory because it made it easier to implement secondary methods such as totalItems() and totalCost() where I needed to iterate through all the items in the inventory and check their values.
- I modified the shoppingList() method to populate a map provided by the user instead of returning a new inventory so that I wouldn't have to compare the cost of each object in each inventory as well and deal with an instance where two inventories have an item with the same name and different prices. Also because item prices aren't typically an important part in shopping lists in real life
